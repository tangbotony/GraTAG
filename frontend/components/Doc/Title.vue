...
function handleApply(event: Event, index: number) {
  ...
  // tracking
  trackForAppy(index)
  const inputElement = props.inputTitle
  inputElement.select()
...
<template>
  <div class="sticky top-0 py-[16px] flex items-center justify-between bg-white z-10 h-[64px] shrink-0">
    <div class="flex items-center">
      <div class="i-ll-edit-h text-[16px] text-black-color" />
      <span class="text-black-color text-[14px] font-normal leading-[22px] ml-[6px]">标题生成</span>
    </div>
    <div class="flex items-center">
      <template v-if="refreshTime < 4">
        <el-tooltip
          content="更新"
        >
  ...
  <template v-else>
    <template v-if="(data.length === 0 && isloading) || isRefreh">
      <div class="w-full h-full flex flex-col items-center justify-center">
        <DocLoading :duration="5" :width="40" @cancle="cancelReq" />
      </div>
    </template>
        ...
        <div v-if="selectedTextIndex === index" class="flex items-center mt-[8px] justify-between">
          <div class="flex items-center">
            <div class="btn mr-[8px] flex items-center cursor-pointer" @click="handleApply($event, index)">
              ...
              <span class="text-[12px] ml-[4px]">应用标题</span>
            </div>
