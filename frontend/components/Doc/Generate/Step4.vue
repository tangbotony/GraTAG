...
function calcRectTop(val: number) {
  if (isInsertTitle.value) {
    ...
    const { top: titleTop, left: leftTitle } = getDistanceToFirstScrollYParent(nextSibling)
...
async function generateArticle() {
  ...
  const res = await useAiGenArticle(props.data, control.value)
  isLoading.value = false

  if (res.data.value?.result) {
    content.value = res.data.value.result.content
    title.value = res.data.value.result.title
    title_list.value.push(res.data.value.result.title)
  }
  else {
    $emits('back')
  ...
  if (!currentUser.value?.extend_data?.generate_step_4) {
    await sleep(600)
    updateUserInfo('generate_step_4', true)
      ...
      steps: [
        ...
        },
        {
          ...
          popover: {
            ...
            <span>重新启动侧边栏修改前置内容</span>
          </div>`,
          },
        },
        {
          element: '#generate-step4-refresh',
          popover: {
            description: '点击此处按钮重新生成标题',
          },
        },
      ],
      onNextClick: async (element, step, options) => {
        if (options.state.activeIndex === 0) {
          const second = document.querySelector('#generate-step4-before')
          second?.scrollIntoView()
        }
        else if (options.state.activeIndex === 1) {
          scrollContainer.value?.scrollTo({
            top: isInsertTitle.value ? rect.value.top : rect.value.top - 20,
            behavior: 'smooth',
          })
        }
        await sleep(200)
...
async function calcBox(refresh = false) {
  ...
  let contetStart = props.editor.state.selection.to
  if (!isInsertTitle.value)
    contetStart = props.editor.state.selection.from - 1

  let end = 0
  if (!refresh) {
    await typewriter(content.value, async (text) => {
...
async function refreshTitle() {
  ...
  control.value = new AbortController()
  const res = await useAiGenTitle({
    ...props.data,
    title_list: title_list.value,
    context: catchTextBetweenError(props.editor, contentRange.value.from, contentRange.value.to),
  }, control.value)
  isLoadingTitle.value = false

  if (res.data.value?.result) {
    title.value = res.data.value.result
    title_list.value.push(res.data.value.result)
    clearState()
    if (!isInsertTitle.value)
      currentArticle.title = ''
    ...
    <Teleport to="body">
      <div class="overlay">
        <div class="loading-container">
          <DocLoading :duration="isLoadingTitle ? 0 : 10" :width="80" text-color="white" @cancle="cancelReq" />
        </div>
      </div>
    </Teleport>
  </template>
