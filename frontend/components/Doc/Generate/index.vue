...
onMounted(async () => {
  ...
  if (data) {
    ...
    writeStyle.value = obj.type
    userTitle.value = obj.title
    event.value = obj.answer
    userArguments.value[0].argument = obj.thesis
    sessionStorage.removeItem('qa-temp-gen-new')
    next()
  }
...
<template>
  ...
  <div class="h-[69px] border-t-1 w-full border-[rgba(217, 217, 217, 1)] flex items-center justify-between px-[24px]">
    <div
