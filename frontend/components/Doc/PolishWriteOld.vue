...
import { WRITE_STYLE_LIST } from '~/consts/editor'
...
function handleStyle(type: string) {
  if (isloading.value) {
    ElMessage.warning('生成中暂不支持换一批')
    return
  }
  oldStyle = style.value
  style.value = type
  refresh()
}
defineExpose({
...
<template>
  ...
  <div class=" sticky top-[64px] z-10 flex items-center pb-[16px] bg-white">
    <div
      v-for="kind in WRITE_STYLE_LIST"
      :key="kind.type"
      class="polish-tag mr-[8px] cursor-pointer"
      :class="{
        'is-active': (style === kind.type),
      }"
      @click="handleStyle(kind.type)"
    >
    ...
    <template v-else-if="diffData.length > 0 && !isloading">
      ...
      >
        ...
        <div v-if="selectedTextIndex === index" class="flex items-center mt-[8px] justify-between">
          ...
          <DocUpDownButton :type="type" :text="data[index]" :selected_content="currentText" :context_above="currentBeforeText" :context_below="currentAfterText" :polish_type="style" @change="handleUpDownChange" />
        </div>
      </div>
    </template>
