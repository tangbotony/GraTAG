...
def get_QA_score(res_i):
    ...
    return {
        ...
        "category": res_i['category'],
        "guidance_type": res_i['guidance_type'],
...
if __name__ == '__main__':
    files = [
        # "0826_all_base_results_base",
        # "0826_all_base_results_rerank_sft",
        # "0826_all_base_results_base_llm_filter_gpt_4o",
        # "0826_all_base_results_rerank_base",
        # "0826_all_base_results_rerank_sft_0829_all",
        "0826_all_base_results_rerank_sft_0829_all_old_database"
    ]
    names = [
        # 'base', 'rerank_sft', 'gpt-4o',
        # "rerank_base",
        # "rerank_sft_0829_all",
        "rerank_sft_0829_all_old_database"
    ]
    ...
    for score_i, file_path in zip(scores, names):
        for bar in [0.5, 0.55, 0.6, 0.65, 0.7, 0.8, 0.9]:
            score_i_bar = copy.deepcopy(score_i)
            for score_i_item, score_i_origin_item in zip(score_i_bar, score_i):
                new_scores = []
                new_origin_scores = []
                if isinstance(score_i_origin_item['scores'], list) and isinstance(score_i_origin_item['origin_scores'], list):
                    for score_j, origin_score_j in zip(score_i_origin_item['scores'][:30], score_i_origin_item['origin_scores'][:30]):
                        if origin_score_j > bar:
                            new_scores.append(score_j)
                            new_origin_scores.append(origin_score_j)
                    score_i_item['scores'] = new_scores
                    score_i_item['origin_scores'] = new_origin_scores
                    if new_scores:
                        score_i_item['avg_score'] = np.mean(new_scores)
                    else:
                        score_i_item['avg_score'] = np.nan
            df = pd.DataFrame(score_i_bar)
            df['平均材料相关度打分_{}_{}'.format(bar, file_path)] = df['avg_score']
            df['材料检索个数_{}_{}'.format(bar, file_path)] = df['scores'].apply(len)
    ...
    # 保存到Excel文件
    excel_file = '0829_comparison_results_new_data_time_bar_old_database.xlsx'
